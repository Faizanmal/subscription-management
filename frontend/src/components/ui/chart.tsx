...
>(({ 
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName as string)}>
          {(labelFormatter as (value: unknown, payload: Array<Record<string, unknown>>) => React.ReactNode)(value, payloadArray)}
        </div>
      )
    }
    ...
    return <div className={cn("font-medium", labelClassName as string)}>{value}</div>
  }, [
    label,
    labelFormatter,
    payloadArray,
          ...
          return (
            ...
            >
              {formatter && item?.value !== undefined && item.name ? (
                (formatter as (value: number, name: string, item: Record<string, unknown>, index: number, payload: Array<Record<string, unknown>>) => React.ReactNode)(item.value as number, item.name as string, item, index, payloadArray)
              ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
                    ...
                    >
                      <div className="grid gap-1.5">
                        ...
                        <span className="text-muted-foreground">
                          {(itemConfig?.label as React.ReactNode) || (item.name as string)}
                        </span>
                      </div>
                      {item.value !== undefined && (
                        <span className="font-mono font-medium tabular-nums text-foreground">
                          {String((item.value as number).toLocaleString())}
                        </span>
                      )}
                    </div>
                  </>
